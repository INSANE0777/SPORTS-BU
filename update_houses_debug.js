const ENDPOINT = "https://fra.cloud.appwrite.io/v1";
const PROJECT_ID = "68fc96990022ec19614a";
const API_KEY = "standard_";
const DATABASE_ID = "68fc96c0000e08dcfce2";
const HOUSES_TABLE_ID = "houses";

const newHouses = [
  {
    $id: "68ffa7a4001ae9e266f9",
    name: "KEEN KICKERS",
    color: "#4ADE80",
    balance: 50000,
    logo: "pic1" // Use shorter name just in case
  },
  {
    $id: "68ffa94f000e9e21cd92",
    name: "MIGHTY BOLTZ",
    color: "#A78BFA",
    balance: 50000,
    logo: "pic2"
  },
  {
    $id: "690652a27beff6535c29",
    name: "SUPER SPADES",
    color: "#22D3EE",
    balance: 50000,
    logo: "pic3"
  },
  {
    $id: "690652a27bf6fa83727f",
    name: "HEROIC HEARTS",
    color: "#F87171",
    balance: 50000,
    logo: "pic4"
  }
];

async function updateHouses() {
  for (const house of newHouses) {
    const url = `${ENDPOINT}/databases/${DATABASE_ID}/collections/${HOUSES_TABLE_ID}/documents/${house.$id}`;
    const { $id, ...data } = house;
    
    console.log(`Updating house ${house.name}...`);
    const response = await fetch(url, {
      method: 'PATCH',
      headers: {
        'X-Appwrite-Project': PROJECT_ID,
        'X-Appwrite-Key': API_KEY,
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ data }) // Wait, Appwrite expects the data in the top level or 'data'?
      // Native Appwrite API for Update Document expects the fields in the top level.
    });

    if (!response.ok) {
      console.error(`Failed to update ${house.name}:`, await response.json());
    } else {
      console.log(`Successfully updated ${house.name}`);
    }
  }
}

updateHouses();
